...
type ModeInfo_Single struct {
	...
	Mode types.SignMode `protobuf:"varint,1,opt,name=mode,proto3,enum=cosmos.tx.signing.v1beta1.SignMode" json:"mode,omitempty"`
}
	...
	for iNdEx < l {
		...
		case 1:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.Mode |= types.SignMode(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
