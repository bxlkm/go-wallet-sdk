...
func (s *SpecV5R1Final) BuildMessage(ctx context.Context, internal bool, _ bool /*_ *ton.BlockIDExt,*/, messages []*Message) (_ *cell.Cell, err error) {
	...
	opCode := uint64(0x7369676e) // auth_signed_external: 0x7369676e,
	if internal {
		opCode = 0x73696e74 // auth_signed_internal: 0x73696e74
	}
	payload := cell.BeginCell().
		MustStoreUInt(opCode, 32).                        // external sign op code
		MustStoreUInt(uint64(walletId.Serialized()), 32). // serialized WalletId
		MustStoreUInt(unix, 32).                          // validUntil
